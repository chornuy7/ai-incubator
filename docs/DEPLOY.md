...
### Выкат MR-290 идёт ПРИ ОСТАНОВЛЕННОМ СЕРВИСЕ — порядок целиком

Обычный выкат обходится без остановки: миграции применяются, потом деплоится код, и в
промежутке работает предыдущая версия уже с новой схемой. MR-290 так выкатывать нельзя, и
причин две.

1. **Задачи модулей переносятся скриптом.** Пока воркеры живы, они пишут в файлы, и
   перенос выйдет с обрывом на полуслове: часть журнала уедет, часть останется.
2. **Время переходит в `timestamptz` прямо в колонке** (`2026-09-02-mr290-time-types.sql`).
   Это единственная миграция задачи, которая не может «писать и в новое, и в старое»: у
   колонки один тип. Предыдущая версия после неё шлёт epoch-мс числом, Postgres читает
   `1787588909052` как дату и отвечает ошибкой — ломаются обращения в поддержку, учёт
   смен, кэш парсера и витрина оплат.

Поэтому сервис останавливается ДО миграций и поднимается уже с новым кодом — окна, в
котором старый код видит новую схему, просто не возникает:

```bash
sudo systemctl stop ai-incubator
node --env-file=.env server/scripts/migrate.mjs --dry     # что применится: должно быть 19 файлов MR-290
...
# выкат кода (bash deploy/deploy-archive.sh) — он поднимет сервис сам
```

Если выкат кода поднимает сервис самостоятельно, отдельный `start` не нужен; если нет —
`sudo systemctl start ai-incubator` последним шагом.
