...
- ☐ **Осталось только в КОДЕ (в окне, с бэкапом):** снять legacy-вход (`authenticate` по
  `users.password_hash`) и dual-write в `users` (create/update/deleteUser), затем применить
  миграцию. До этого `users` — точка отката, не трогаем.
