...
- ☑ **Код этапа 4 сделан (03.08):** dual-write в `users` снят (create/update/deleteUser),
  вход — только через Supabase Auth. Legacy scrypt-вход оставлен ТОЛЬКО под аварийным
  флагом env `AUTH_ALLOW_LEGACY=1` (вернуть вход без отката кода при неожиданном локауте).
  Полный сет тестов — 566/566, tsc чисто.
- ☐ **Осталось (действия человека, НЕ код):**
  1. Выкатить этот код на прод и перезапустить бэкенд.
  2. **Проверить вход** владельца и Ильи (только через Auth). Если кто-то не заходит —
     env `AUTH_ALLOW_LEGACY=1` + рестарт, разобраться с паролем в Auth, потом снять флаг.
  3. Прожить неделю без инцидентов (users — точка отката: revert кода вернёт legacy-вход,
     таблица ещё на месте).
  4. Бэкап → применить `2026-08-03-drop-users-stage4.sql` (вариант А — переименовать).
