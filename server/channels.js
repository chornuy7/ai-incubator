...
const channelToRow = (c) => {
  ...
  for (const [key, v] of Object.entries(c)) {
    // Списки живут строками в отдельных таблицах — в мешок им нельзя даже временно,
    // иначе один и тот же список будет и там и там, и однажды они разойдутся.
    if (COLUMN_KEYS.has(key) || key === 'sources' || key === 'categoriesExtra') continue
    row.data[key] = v
  }
