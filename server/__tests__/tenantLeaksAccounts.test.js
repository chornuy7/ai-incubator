...
test('глобальные настройки ИИ и чёрный список меняет только админ', async () => {
  // Один ЧС и одни лимиты ИИ-безопасности на всю платформу: запись без гейта означает,
  // что любой клиент правит поведение и списки целей у ВСЕХ остальных.
  //
  // `/ai-settings` из этого списка убран 27.08 ОСОЗНАННО: системный промпт перестал быть
  // общим и стал личным (MR-185). Требовать на него админский гейт значило бы запретить
  // человеку менять собственный промпт. Взамен он охраняется по-другому — владелец берётся
  // из сессии; это сторожит отдельный тест ownerPromptWiring.
  const src = await fs.readFile(srv('featureRoutes.js'), 'utf8')
  const global = ['/ai-safety', '/target-blacklist']
