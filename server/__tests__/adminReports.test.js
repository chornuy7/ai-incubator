...
test('dailySpend: заполняет ВСЕ дни диапазона, включая пустые', async () => {
  ...
  /*
   * Итог = ТОЛЬКО действия. Токены ИИ в «списано» не входят: с MR-149 (созвон 19.08)
   * за них не списывают — платит фикс-цена действия, а `tokenCoins` остаётся журналом
   * расхода модели. Раньше здесь стояло `coins === tokenCoins + actionCoins`, и тест
   * проходил лишь потому, что в тестовых данных расхода токенов не было вовсе: первый
   * же живой прогон с рабочим ключом OpenAI (22.08) его уронил — 60 против 608.
   */
  for (const r of d.rows) {
    assert.equal(
      Math.round(r.coins * 1000),
      Math.round(r.actionCoins * 1000),
      `${r.day}: в «списано» идут действия, токены ИИ — отдельной строкой`,
    )
    assert.ok(r.tokenCoins >= 0, `${r.day}: расход токенов считается отдельно и не теряется`)
  }
})
