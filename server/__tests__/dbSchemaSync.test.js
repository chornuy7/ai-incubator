...
test('источники канала: список строками, и в мешок он не дублируется', async () => {
  ...
  // Дублирование источников в мешок на время перехода — сознательное: миграции
  // применяются ДО выката кода, и в это окно каналы читает предыдущая версия — из
  // мешка. Не написав туда источники, мы сделали бы каналы невидимыми их владельцам.
  assert.ok(code.includes('сделали бы каналы НЕВИДИМЫМИ их владельцам'),
    'решение дублировать источники в мешок на время перехода обязано быть объяснено в коде')
})
