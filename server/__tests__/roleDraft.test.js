...
test('повторное нажатие на выбранную роль снимает выбор и прячет редактор', () => {
  /*
   * Просьба владельца 02.09: «при нажатии ещё раз на роль, которую создали, этот блок
   * справа должен прятаться и становиться неактивной роль, которая у нас есть».
   *
   * Выбранную роль нельзя было отпустить: форма на восемьдесят тумблеров висела справа до
   * перезагрузки страницы, а закрыть её было нечем — кнопки «закрыть» у редактора нет, и
   * единственным выходом был выбор ДРУГОЙ роли.
   */
  const src = читать('src/pages/RolesPage.tsx')
  assert.match(src, /onClick=\{\(\) => \(selId === r\.id \? cancelDraft\(\) : selectRole\(r\)\)\}/,
    'повторное нажатие обязано снимать выбор, а не перевыбирать ту же роль')
  // Редактор рисуется только при выбранной роли — значит снятие выбора его и прячет.
  assert.match(src, /\{selected && \(/)
  // А снятие выбора — это пустой selId: иначе строка осталась бы подсвеченной.
  const тело = src.slice(src.indexOf('function cancelDraft()'), src.indexOf('async function removeRole'))
  assert.match(тело, /setSelId\(''\)/)
})
