...
test('выпуск отдаёт полный ключ один раз, список — только префикс', async () => {
  const { K, f } = await fresh()
  const issued = await K.issueKey({ name: 'Мозги' })
...
test('verifyKey: валидный проходит, отозванный и чужой — нет', async () => {
  const { K, f } = await fresh()
  const issued = await K.issueKey({ name: 'X' })
  assert.ok(await K.verifyKey('Bearer ' + issued.key), 'валидный проходит')
