...
test('CRUD ролей на изолированном файле + сид по умолчанию', async () => {
  ...
  // Первое чтение сидит дефолты: Администратор + Оператор + Модератор + Sales + Viewer + Поддержка.
  ...
  assert.deepEqual(
    ...
    ['role_moderator', 'role_operator', 'role_sales', 'role_support', 'role_viewer'],
  )
  ...
  assert.equal((await r.listRoles()).length, 7)
