...
test('экран, который НЕ меняется после нажатия, — проверка «я не робот», а не шаг', async () => {
  /*
   * Дамп живого диалога 02.09 (аккаунт smurfs Richardson, прод):
   *
   *   «This is a mistake» → «Yes» → «No! Never did that!»
   *   → «Please verify you are a human.» + единственная кнопка «Done»
   *   → отправляем «Done» → приходит ТОТ ЖЕ экран. Трижды подряд, девять одинаковых
   *     сообщений в диалоге.
   *
   * Значит анти-бот проверка на последнем шаге ЕСТЬ и текстовым нажатием не проходится —
   * Telegram её для того и поставил. Правка 01.09 утверждала обратное («капчи там не
   * было») и была неверна; первоначальная постановка MR-192 права.
   *
   * Правило структурное: сравниваем подпись экрана (текст + набор кнопок), а не слова, —
   * иначе португальский и персидский ответы, которые уже приходили с прода, снова мимо.
   */
  const код = fs.readFileSync(new URL('../lib/spamAppeal.js', import.meta.url), 'utf8')
  assert.match(код, /function подписьЭкрана\(msg\)/, 'нужна структурная подпись экрана')
  assert.match(код, /подписьЭкрана\(msg\) === прежняя/, 'повтор экрана обязан распознаваться')
  assert.match(код, /state: 'captcha'[\s\S]{0,40}appealed: true/, 'жалоба заполнена — это не «не довели»')

  // И оператору говорим, ЧТО делать, а не «просит проверку».
  const модуль = fs.readFileSync(new URL('../spamUnblock.js', import.meta.url), 'utf8')
  assert.match(модуль, /остался последний шаг/)
  assert.match(модуль, /вручную под этим аккаунтом/)
})
