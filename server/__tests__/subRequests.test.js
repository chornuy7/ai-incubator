...
test('«я сотрудник» берётся из свежего баланса, а не из зашитой сессии', async () => {
  /*
   * Приёмка 31.08: «обновил страницу — и пропала кнопка». Признак сотрудника жил в двух
   * местах: в сессии (зашита при входе и дальше не меняется) и в балансе (сервер отдаёт
   * каждый раз). После MR-225 сотрудник со своим кошельком переставал считаться
   * сотрудником до перезахода — панель показывала то одно, то другое.
   *
   * Источник должен быть один и свежий. Сессия остаётся запасным вариантом на случай,
   * когда баланс ещё не загрузился.
   */
  const fs2 = await import('node:fs/promises')
  for (const [файл, где] of [
    ['../../src/pages/SupportPage.tsx', 'страница обращений'],
    ['../../src/widgets/SupportWidget.tsx', 'виджет поддержки'],
  ]) {
    const src = await fs2.readFile(new URL(файл, import.meta.url), 'utf8')
    assert.ok(/useBalance\(\)/.test(src), `${где}: признак должен приходить из баланса`)
