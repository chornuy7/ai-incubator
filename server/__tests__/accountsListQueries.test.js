...
test('сборка строк прокси идёт через кэш каталога', () => {
  // listProxiesWithSecrets читает таблицу и расшифровывает все пароли. withProxyStrings
  // зовётся на КАЖДОЕ чтение меты, поэтому здесь допустим только кэшированный вход.
  const код = читать('../lib/proxyLink.js')
  assert.ok(!код.includes('listProxiesWithSecrets('), 'нужен proxyCatalog — кэш написан ровно под это место')
  assert.ok(код.includes('await proxyCatalog()'))
})
