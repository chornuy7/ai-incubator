...
const LABEL = {
  ...
  /*
   * Настоящая проверка — та, что уводит из диалога ссылкой или картинкой. Такое Telegram
   * пока не присылал (правка 01.09: под словами «verify you are a human» приходит обычная
   * кнопка «Done», её модуль теперь жмёт сам). Ветка остаётся на случай, если пришлёт.
   */
  captcha: '@SpamBot просит пройти проверку по ссылке — завершите вручную',
