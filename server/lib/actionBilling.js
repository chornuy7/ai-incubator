...
/**
 * Списать за N выполненных действий и остановить задачу, если монеты кончились.
 *
 ...
 * уходит, поэтому на нуле задачу останавливаем сами — иначе правило «при нуле
 * модули стоят» действовало бы только на запуске, а начатая задача доработала бы
 * даром.
 *
...
export async function chargeActions(task, store, actions = 1, deps = {}) {
  try {
    ...
    if (coins <= 0 && !task.stopRequested) {
      task.stopRequested = true
      await store?.appendLog?.(task, 'error', 'Закончились монеты — задача остановлена. Пополните баланс и запустите заново.')
    }
