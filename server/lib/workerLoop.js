...
export function warmingPace(level) {
  ...
  /*
   * Правка 26.08 (владелец: «ему же нужна какая-то база каналов… рандом полный должен
   * быть: зайдёт, посмотрит, реакцию кинул, где-то подписался, где-то отписался»).
   *
   * Было: 40% «просмотр», который на деле выполнял ПОИСК каналов и ничего не открывал,
   * плюс 20% чтения списка диалогов и 10% getMe. То есть 70% действий были служебными
   * вызовами API, а не поведением человека.
   *
   * Стало: просмотр реально открывает посты канала, добавлены отписка (профиль, который
   * только вступает и никогда не выходит, выглядит роботом) и заметка себе в «Избранное».
   */
  const weights = { view: 30, react: 20, read: 15, join: 12, leave: 8, note: 5, ping: 10 }
  const plans = [
...
/**
 * Сколько часов в сутки аккаунт греется — выбор человека (просьба владельца 27.08:
 * «добавить выбор: 6 часов прогрева каждый день, 8 или 12; в них распределяются
 * действия одного аккаунта»).
 *
 * Окно решает не сколько действий, а насколько они растянуты: те же 20 действий за 6
 * часов идут раз в 18 минут, за 12 часов — раз в 36. Узкое окно похоже на человека,
 * который заходит вечером; широкое — на того, кто весь день в телефоне. Пусто —
 * прежние четырнадцать часов.
 */
export function warmWindowMs(hours) {
  const h = Math.max(1, Math.min(24, Number(hours) || 0))
  return h ? h * 60 * 60 * 1000 : WARM_WINDOW_MS
}
