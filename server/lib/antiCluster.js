 ...
 *
 * ВАЖНО: основную гарантию даёт не эта функция, а глобальный лок «один аккаунт =
 * одна задача» (accountLocks.js) — он уже не даёт профилю работать в двух модулях
 * разом. Держим правило отдельно как страховку на случай, если появится модуль,
 * ведущий несколько чатов внутри одной задачи: тогда считать параллельность будет
 * нечем, а требование §4.4 «не пишет в 5 группах» останется в силе.
