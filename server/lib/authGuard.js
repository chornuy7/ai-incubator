...
const PUBLIC = [
  ...
  // Конфиг капчи нужен СТРАНИЦЕ ВХОДА И РЕГИСТРАЦИИ, то есть человеку, который ещё не
  // вошёл. Без него фронт не знает site-key, виджет Turnstile не рисуется, токен не
  // приходит — и регистрация отвечает «проверка «я не робот» не пройдена» всем подряд.
  // Отдаём только флаг и ПУБЛИЧНЫЙ site-key; секрет остаётся на сервере.
