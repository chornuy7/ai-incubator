...
rolesRouter.get('/', async (req, res) => {
  try {
    ...
    /*
     * Владелец видит ТОЛЬКО СВОИ созданные роли (уточнение 21.08: «показывать только
     * созданные роли»). Раньше сюда подмешивались системные шаблоны — и список
     * начинался с чужих «Оператор», «Sales», «Viewer», среди которых собственные
     * терялись; на живом стенде их набралось 43 штуки с тройными дублями.
     *
     * Персональные роли субов (personalFor) тоже прячем: их заводит карточка
     * пользователя, а не человек, и в списке шаблонов им делать нечего.
     * Админу платформы отдаём всё — ему нужна полная картина прав.
     */
    const visible = (ctx.noSession || ctx.isAdmin)
      ? roles
      : roles.filter((r) => r.id !== ADMIN_ROLE_ID && !r.personalFor && r.userId === ctx.id)
