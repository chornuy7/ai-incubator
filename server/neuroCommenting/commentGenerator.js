...
/**
 * Правило поверх ЛЮБОГО промпта — карточки, глобального, своего текста.
 *
 * Живой прогон 24.08: на пост из двух слов («test 2») модуль выдал «Сообщение содержит
 * лишь тестовый текст, что затрудняет анализ его содержания… Рекомендуется предоставить
 * более детальную информацию». Это не комментарий, а отзыв рецензента о посте — так
 * читатель канала не пишет никогда, и по такой реплике аккаунт виден сразу.
 *
 * Держим здесь, а не в тексте промпта: промпты редактирует оператор, и правило про
 * «не рецензировать пост» не должно теряться при каждой правке.
 */
const STYLE_GUARD = [
  'Пиши как обычный читатель канала, от первого лица.',
  'Не пересказывай пост и не оценивай его качество, информативность или полноту.',
  'Не давай автору советов о том, что и как ему публиковать.',
  'Не начинай ответ со служебных слов вроде «Комментарий:» или «Ответ:».',
  'Если по сути сказать нечего — ответь коротко и по-человечески: эмоция, короткий вопрос или пара слов.',
].join(' ')
