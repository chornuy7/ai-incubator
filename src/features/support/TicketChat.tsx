...
function authorLabel(m: TicketMessage, ticket: ApiTicket): string {
  if (m.from === 'support') {
    /*
     * Приёмка 31.08: сотрудник спросил токены у своего администратора и получил ответ,
     * подписанный «Поддержка». Сервер подписывает такой ответ ИМЕНЕМ ВЛАДЕЛЬЦА (MR-257) —
     * подпись терялась здесь, на отрисовке.
     *
     * Разница по адресату обращения: у обращения к владельцу (`toOwnerId`) отвечающая
     * сторона — конкретный человек, которого сотрудник знает по имени. У обращения к нам
     * отвечает роль, и личный контакт оператора клиенту не нужен, — там «Поддержка».
     */
    if (ticket.toOwnerId) return m.authorName || m.authorEmail || 'Администратор'
    return 'Поддержка'
  }
