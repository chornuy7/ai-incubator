...
export function AppHeader() {
  ...
  /** MR-257: запрос токенов у владельца — сотруднику больше некуда идти за ними. */
  const [запросТокенов, setЗапросТокенов] = useState(false)
  ...
  for (const w of wallet) {
    ...
    /*
     * Покупка и докупка модулей. Раньше сюда не попадала ни одна: под «продлени» она не
     * подходит, а последняя ветка ловит только ПЛЮС на счёте. Списание за покупку уходило
     * молча — заказчик 31.08, купив подписку: «чого увідомленія немає як було раніше».
     *
     * Человек только что нажал кнопку сам, но узнать сумму ему всё равно неоткуда: тост
     * покупки суммы не называет, а колокольчик — единственное место, где операция
     * остаётся видимой после перезагрузки страницы.
     */
    if (/^подписка[:\s]/i.test(w.reason) && w.currency === 'usd' && w.amount < 0) {
      notifItems.push({ key: `wallet:${w.ts}`, tone: 'yellow', title: `Списано ${подпись}`,
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-surface/80 backdrop-blur-xl">
      ...
      <RequestToOwnerModal kind="tokens" open={запросТокенов} onClose={() => setЗапросТокенов(false)} />
        ...
        */}
        <div className={'mb-4 grid grid-cols-1 gap-3' + (balance?.isSub ? '' : ' sm:grid-cols-2')}>
          ...
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3.5">
            ...
            {/*
              MR-204/MR-257: на месте убранной покупки у сотрудника — запрос владельцу.
              Раньше здесь было ПУСТО: блоки «Пополнить счёт» и «Купить токены» сотруднику
              не показываются (он не распоряжается деньгами владельца), но и попросить их
              было негде — приходилось идти во внешний мессенджер.
            */}
            {balance?.isSub && (
              <button
                type="button"
                onClick={() => setЗапросТокенов(true)}
                className="btn-soft mt-2 h-8 w-full justify-center text-xs"
              >
                Запросить токены у администратора
              </button>
            )}
          </div>
        </div>
