 ...
 */
/**
 * Заливка ТОЧКИ статуса (легенда чарта, заголовок группы) — кружок 10 px.
 *
 * Цвет здесь сплошной, а не подложка, поэтому зебра берётся в полную силу. Шаг мельче, чем
 * у тега: там полоса идёт через всю плашку, а тут — поперёк кружка, и шаг тега (5 px) дал бы
 * одну диагональную границу, то есть кружок «разрезанный пополам», а не полосатый. При 1.5 px
 * поперёк помещается четыре-пять полос — читается как штриховка и не рябит.
 */
export function statusDotFill(tone: TaskStatusTone): string {
  if (!tone.zebra) return tone.base
  return `repeating-linear-gradient(45deg, ${tone.base} 0 1.5px, ${withAlpha(tone.base, 0.28)} 1.5px 3px)`
}
