...
export const MODULES: Record<string, ModuleConfig> = {
  'neuro-commenting': {
    ...
    // Правка 18.08. Было три группы, которые спорили друг с другом: «Случайный / По
    // ключевым словам / Все посты» и «Только новые / Только существующие / Все посты» —
    // «Все посты» дважды, а «случайный» и «только новые» звучали как взаимоисключающие,
    // хотя отвечают на разные вопросы. Разделено по смыслу: ЧТО подходит (содержание) и
    // СКОЛЬКО последних постов смотрим (глубина). «Сколько брать из подходящих» — галочка.
    toggleGroups: [
      { label: 'Что комментировать', options: ['Любые посты', 'По ключевым словам'] },
      { label: 'Режим работы', options: ['По количеству', 'По времени'] },
      // Без своего заголовка: рисуется вторым рядом под «Что комментировать».
      { label: '', options: ['Только последний', 'Последние N', 'Все доступные'] },
    ],
