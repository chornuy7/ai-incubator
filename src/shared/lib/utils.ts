...
/**
 * Остаток на счету — для строк истории операций.
 *
 * MR-230: в истории стояло «10111.00 → 11511.00» — два голых числа, по которым не понять
 * ни что это остаток, ни в чём он. Заказчик 30.08: «тут не дуже подається, як зміна балансу
 * йде». Отличий от `coins` два: разряды разделены (10 111, а не 10111) и «.00» не пишется,
 * когда число целое. Дробную часть сохраняем — списания бывают по 0.005.
 *
 * Разделитель — обычный пробел, а не запятая из toLocaleString: в остальном интерфейсе
 * дробная часть отделяется точкой, и смешивать два стиля в одной строке нельзя.
 */
export function balance(n: number): string {
  const s = coins(n).replace(/\.0+$/, '')
  const [целая, дробная] = s.split('.')
  return целая.replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + (дробная ? '.' + дробная : '')
}
