...
/**
 * Страницы управления доступом: роли и пользователи. Обычной роли они закрыты; единственное
 * исключение — «Команда» у владельца пространства (OWNER_TEAM_PATH). Подписка сюда НЕ входит:
 * клиент сам заходит в «Мои модули» и покупает свой набор — личная покупка
 * перекрывает общий набор только для него. §8.1 / §5.4
 *
 * Утром 21.08 сюда добавляли исключение и для «Ролей и доступов» (OWNER_ROLES_PATH) —
 * снято по уточнению владельца от 21.08: отдельной страницы ролей у владельца быть не
 * должно, доступ субу он настраивает в карточке пользователя («Пользователи» → «Доступ к
 * модулям»), а персональную роль суба заводит и правит сервер по PUT /api/users/:id/access.
 * /panel/roles остаётся страницей админа платформы.
 */
...
export function canAccessPath(
  ...
  if (isAdmin) return true
  // §4.1 (MR-29): владельцу открыта только «Команда» из админ-страниц; «Роли» — по-прежнему
  // sudo. Уточнение владельца от 21.08: решать, что видно субу, он всё так же должен сам, но
  // делает это внутри карточки суба на «Команде», а не отдельной страницей ролей.
