 ...
 * для владельца пространства — ниже (OWNER_TEAM_PATH / OWNER_ROLES_PATH). Подписка сюда НЕ входит:
...
/**
 * Решение владельца 21.08: «Роли и доступы» открыты не только sudo-админу, но и владельцу
 * пространства — раздавать субам доступ к СВОИМ оплаченным модулям должен он сам, а не
 * платформа. Сервер это уже поддерживает: владельцу отдаются системные шаблоны + его роли
 * (админ-роль скрыта), менять/удалять он может только свои, а каталог модулей ограничен
 * его подпиской. Суб сюда не ходит: своих субпользователей у него нет.
 */
...
export function canAccessPath(
  ...
  if (isAdmin) return true
  // §4.1 (MR-29) + правка 21.08: владельцу открыты «Команда» и «Роли и доступы» — он
  // набирает субов и он же решает, что каждому из них видно. Остальные админ-страницы
  // остаются за sudo.
  if (ADMIN_ONLY_PATHS.has(path)) return isOwner && (path === OWNER_TEAM_PATH || path === OWNER_ROLES_PATH)
