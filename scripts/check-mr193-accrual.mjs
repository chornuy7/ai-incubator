...
const MODULES = ['mailing', 'neuro-commenting']
...
try {
  ...
  провал = !ok((gifts || []).length === MODULES.length, '11. факт выдачи записан по каждому модулю',
    (gifts || []).map((g) => g.module_key).join(', ')) || провал
