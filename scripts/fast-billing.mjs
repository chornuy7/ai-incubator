...
for (let i = 1; i <= ШАГОВ; i++) {
  // «Сейчас» = день списания того месяца, в котором подписка заканчивается, дальше по
  // месяцу за шаг. Начинать позже нельзя: тик намеренно не воскрешает подписки, брошенные
  // больше трёх суток назад, и первый же шаг ушёл бы мимо окна продления.
  const дата = new Date(Date.UTC(база.getUTCFullYear(), база.getUTCMonth() + i - 1, sub0.billing_day, 12, 0, 0))
