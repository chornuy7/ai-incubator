...
Раньше SQL-файлы из `supabase/migrations/` копипастили в SQL Editor руками. Работает
ровно до первой забытой миграции — а забытая миграция это не «неудобно», это разъехавшаяся
схема на проде, которую замечают по странным ошибкам через неделю. Теперь база сама помнит,
что применено (таблица `schema_migrations`).

Подключение — ПРЯМОЙ Postgres, потому что Data API не умеет DDL: сервисный ключ ходит по
таблицам, а `create table` через него не выполнить. Строка: Supabase → **Connect** →
**Direct** → Connection string (если не подключается по IPv4 — **Session pooler**).
В локальный `.env`, не в git:

```
SUPABASE_DB_URL=postgresql://postgres:ПАРОЛЬ@db.<ref>.supabase.co:5432/postgres
```

**Обычно ничего запускать не надо: миграции гоняет деплой.** Шаг стоит в `deploy.yml`
ДО выката кода — чтобы код, ожидающий новой колонки, не приехал раньше самой колонки.
Нужен секрет `SUPABASE_DB_URL` (Settings → Secrets and variables → Actions); пока его нет,
шаг тихо пропускается. Руками — вкладка Actions → workflow **migrate** (dry / baseline / apply).

⚠️ **Что уже накатано руками — записано в [`baseline.txt`](./baseline.txt).** Часть наших
миграций не идемпотентна, среди них есть `drop` (например `2026-08-03-drop-users-stage4.sql`):
переприменить их на живой базе значит снести данные. Поэтому при пустом учёте прогонятор
отмечает файлы ИЗ ЭТОГО СПИСКА как применённые, не выполняя их, а всё остальное применяет
обычным порядком. Список лежит в git и виден в ревью — это не догадка «база выглядит
непустой, наверное всё накатано». **Дописывать в него ничего не нужно:** новые миграции
едут сами.

Локально (если у сети открыт порт 5432 — у нас он закрыт, см. ниже):

```bash
npm run migrate -- --dry        # показать, что будет применено
npm run migrate                 # применить неприменённое
npm run migrate -- --baseline   # отметить всё нынешнее накатанным, ничего не выполняя
```

⚠️ **В секрет GitHub кладите строку Session pooler, а не Direct.** Direct connection у
Supabase живёт только в IPv6, а у GitHub-раннеров IPv6 нет — подключение падает с
`ENETUNREACH` на адрес вида `2a05:...`. Session pooler ходит по IPv4 и поддерживает
транзакции, которые нужны миграциям (в отличие от Transaction pooler на 6543):

```
postgresql://postgres.<ref>:ПАРОЛЬ@aws-0-<регион>.pooler.supabase.com:5432/postgres
```

Обратите внимание на имя пользователя: у пулера это `postgres.<ref>`, а не просто `postgres`.

**Почему через раннер, а не с машины.** Прогонятор ходит к Postgres напрямую, портом 5432,
и это не HTTP. На машине владельца весь внешний DNS заворачивается на локальный прокси
(любой хост резолвится в `127.225.0.x`), наружу проходят только 80 и 443 — HTTPS к Supabase
отвечает, а Postgres нет. У GitHub-раннера сеть настоящая.

Файл, изменённый ПОСЛЕ применения, прогонятор не переприменяет молча — он предупреждает:
в базе то, что накатили раньше, и расхождение надо разбирать руками.
