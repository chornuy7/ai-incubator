...
--
-- ⚠️ ЭТОТ ФАЙЛ — ЕДИНСТВЕННОЕ ИСКЛЮЧЕНИЕ ИЗ ПРАВИЛА «ПИШЕМ И В НОВОЕ, И В СТАРОЕ».
--
-- Остальные миграции MR-290 заводят новое рядом со старым, и предыдущая версия кода
-- переживает окно между накаткой миграций и выкатом кода. Здесь так нельзя физически: у
-- колонки один тип. После `alter column ... type timestamptz` предыдущая версия шлёт в неё
-- epoch-мс числом, Postgres читает `1787588909052` как дату и отвечает ошибкой — то есть
-- ломаются обращения в поддержку, учёт смен, кэш парсера и витрина оплат.
--
-- Завести 26 колонок-двойников `*_ts` рядом и снести старые следующим выпуском было бы
-- честнее по форме, но означало бы 26 лишних колонок, двойную запись в каждом сторе и
-- второй такой же переезд через выпуск — ради окна в несколько минут.
--
-- Поэтому решение другое и оно записано в docs/DEPLOY.md: ВЫКАТ MR-290 ИДЁТ ПРИ
-- ОСТАНОВЛЕННОМ СЕРВИСЕ. Остановка там всё равно нужна — задачи модулей переносятся
-- скриптом, и при живых воркерах перенос выйдет с обрывом на полуслове. Окна просто не
-- возникает: сервис поднимается уже с новым кодом и новой схемой.
--
