...
> ⚠️ **К применению на проде (31.07, CRM):** перед выкатом свежего кода из
> `feat/a-phase1-status-machine` выполни в Supabase → SQL Editor файл
> **`2026-07-31-lead-source.sql`** (одна строка: `alter table leads add column ... task_id`).
> Без него запись лида (`upsertLead`) упрётся в несуществующую колонку `task_id` и
> сломается — лиды перестанут создаваться. Миграция аддитивная: применить можно
> и заранее, приложение работает до и после. Порядок: **сначала SQL → потом выкат кода.**
...
| 11 | `2026-07-31-lead-source.sql` | `leads.task_id` (+ индекс) | CRM: «откуда пришёл лид» по задаче — **применить до выката CRM-кода** |
