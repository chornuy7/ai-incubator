...
--
-- ⚠️ ЧЕСТНО ПРО ГРАНИЦУ ЭТОЙ МИГРАЦИИ. Она закрывает расхождение в ОДНУ сторону — объекты,
-- которые есть на проде и которых нет в миграциях. Обратное расхождение остаётся, и одним
-- файлом его не убрать: `2026-07-30-owner-and-types.sql` правит таблицу `parsed_channels`,
-- `2026-07-31-rls.sql` — `api_keys`, и ни ту, ни другую не создаёт ни одна миграция
-- (проверено запросом к базе: обе отвечают «relation does not exist»; parsed_channels
-- позже ещё и дропнута в 2026-08-30-mr190-cleanup.sql). Прогон с нуля упрётся туда.
--
-- Починить это правкой тех файлов нельзя: они применены, а прогонятор считает контрольные
-- суммы и на изменённом файле останавливается — и правильно делает. Поэтому обе ссылки
-- внесены списком известных долгов в `server/__tests__/migrationOrder.test.js`, а тест
-- сторожит, чтобы новых таких не появлялось.
--
-- Штатный способ поднять вторую базу остаётся прежним и описан в шапке
-- `server/scripts/migrate.mjs`: снимок `supabase/schema.sql` плюс `--baseline`. Цепочка с
-- нуля не проигрывается по замыслу — среди миграций есть `drop`, и переприменить их на
-- живой базе значит снести данные.
--
